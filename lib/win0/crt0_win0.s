...
;
;  Entry at 0x0180 (first in _CODE, == WIN0_CODE origin in win0.mk), reached
;  from the stage-1 loader with:
...
;
;  ABI: SDCC 4.5.0 (__sdcccall(1)). main() returns its int value in DE, so the
;  DSS.Exit path takes the exit code from E (cf. lib/crt0.s).
; =========================================================================
...
_win0_entry::
        ...
        ; SDCC 4.5.0 returns int in DE; exit code goes in B.
        ld      b, e
        ld      c, #0x41
        rst     #0x10
        ; (does not return)
        ...
        ; ----- Global initialization (mirrors lib/crt0.s for SDCC 4.5.0) -----
        .area   _GSINIT
gsinit::
